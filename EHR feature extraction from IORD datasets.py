...
for index_date in pd.date_range(start_date, end_date, freq='1440T'):
    ...
    LOS_features=inpatient_epidsodes_day_t_previous365.groupby(['ClusterID'])['LOS, hour'].agg(['max', 'min', 'mean', 'median', 'std'])
    LOS_features.columns=['Maximum LOS of prior admissions within 365 days before index date, hour', 'Minimum LOS of prior admissions within 365 days before index date, hour', 'Mean LOS of prior admissions within 365 days before index date, hour', 'Median LOS of prior admissions within 365 days before index date, hour', 'LOS SD of prior admissions within 365 days before index date']
    ...
    for hourMarker in hourMarkers:
        ...
        for test in tests:
            ...
            if len(test.split(','))>1:
                labs_day_t_temp_stats_test.columns=['Number of '+test.split(',')[0]+' '+str(hourMarker)+' hours before index date', 'Maximum '+test.split(',')[0]+' '+str(hourMarker)+' hours before index date,'+test.split(',')[1], 'Minimum '+test.split(',')[0]+' '+str(hourMarker)+' hours before index date,'+test.split(',')[1], 'Mean '+test.split(',')[0]+' '+str(hourMarker)+' hours before index date,'+test.split(',')[1], 'Median '+test.split(',')[0]+' within current admission,'+test.split(',')[1], 'SD of '+test.split(',')[0]+' '+str(hourMarker)+' hours before index date']
            else:
...
cols = ['Male gender (Yes/No)', 'Female gender (Yes/No)']
...
# EthnicGroup
Database['Ethnic: White (Yes/No)']=np.where(Database['EthnicGroupCode'].isin(['A ','B ','C ']), 1, 0)
Database['Ethnic: Mixed (Yes/No)']=np.where(Database['EthnicGroupCode'].isin(['D ','E ','F ','G ']), 1, 0)
Database['Ethnic: Asian or Asian British (Yes/No)']=np.where(Database['EthnicGroupCode'].isin(['H ','J ','K ','L ']), 1, 0)
Database['Ethnic: Black or Black British (Yes/No)']=np.where(Database['EthnicGroupCode'].isin(['M ','N ','P ']), 1, 0)
Database['Ethnic: Other Ethnic Groups (Yes/No)']=np.where(Database['EthnicGroupCode'].isin(['R ','S ']), 1, 0)
Database['Ethnic: Not stated/Not known (Yes/No)']=np.where(Database['EthnicGroupCode'].isin(['Z ',99]), 1, 0)
cols = ['Ethnic: White (Yes/No)', 'Ethnic: Mixed (Yes/No)', 'Ethnic: Asian or Asian British (Yes/No)', 'Ethnic: Black or Black British (Yes/No)',
       'Ethnic: Other Ethnic Groups (Yes/No)', 'Ethnic: Not stated/Not known (Yes/No)']
Database['Ethnic group']=Database[cols].idxmax(1)
# IMDScore
...
for code in list(set(PostCodes["Fixed_Postcode"])):
    Database[code+' (Yes/No)'] = np.where(Database['PostcodeStub'].isin(PostCodes['PostcodeStub'][PostCodes["Fixed_Postcode"]==code].values.tolist()), 1, 0)
...
# Admission source
Database['Admission source: Usual place of residence']=np.where(Database['AdmissionSourceCode'].isin([29,'29', 19,'19']), 1, 0)
Database['Admission source: Non-NHS institutional care']=np.where(Database['AdmissionSourceCode'].isin([66, '66', 69,'69', 65, '66', 54, '54', 88, '88', 87, '87', 89, '89', 86, '86', 85, '85']), 1, 0)
Database['Admission source: Other NHS Provider']=np.where(Database['AdmissionSourceCode'].isin([51, '51', 52,'52', 53, '53', 49, '49']), 1, 0)
cols = ['Admission source: Usual place of residence', 'Admission source: Non-NHS institutional care', 'Admission source: Other NHS Provider']
